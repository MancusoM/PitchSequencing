...
from calculations.calculate_sequencing import define_additional_cols,create_pitch_sequencing,count_combinations
import polars as pl
...
for team in team_list[0:10]:
    ...
    try:
        data = pl.from_pandas(statcast(opening,closing,team)[
                ['player_name', 'game_date', 'outs_when_up', 'batter', 'inning', 'pitch_number', 'pitcher', 'stand',
                 'pitch_type', 'zone']],
            schema_overrides={
                "zone": pl.Int32,
                "batter": pl.Int32,
        ...
        raise ConnectionError("maybe timed out")
    if len(data.columns)==0:
        raise ValueError("Ensure team abbrev is correct")
    ...
    for i in ['pitch_type','pitch_zone_combo','pitch_group','pitch_group_combo']:
        ...
        team_combinations = combinations.join(players,how = "left",left_on="Pitcher",right_on='MLBAMID')[['Pitcher','Name','Pitch 1','Pitch 2','Amount']].with_columns(
            pl.lit(team).alias("Team")
        ).with_columns(
            pl.lit(i).alias("Call")
        )
        teams_df = pl.concat([teams_df,team_combinations])
    now = datetime.now()
